...
class MenuCtrl {
   ...
   */
  setupTemplate() {
    let title = dom.create("h1", "-wrap highlight unselectable", "#@name_app"),
      intro = dom.create("p", "menu-intro", "#@description");
    this.title = title;
    ...
    this.difficultyOption = new OptionCtrl([
      {
        value: config.GAME.DIFFICULTY.EASY,
        label: "#@button_easy",
        default: true,
      },
    ...
    this.el = dom.create("div", "menu-layout view", [
      dom.create("div", "view-bloc menu-layout-instructions", [
        dom.create("div", "ui-row", [title, langButton]),
        intro,
        langSelector.el,
        instructions.el,
      ]),
